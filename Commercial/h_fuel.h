...
//	get_table_index(&db,"Config","VEHIINTKEY",&int_key);
	...
	for(count=1;count<=max_count*2;count++)
	{
		...
		ADDVALUE("FUEL_TYPE","''",count); //12,1
		ADDVALUE("GRADE","",count); //-6,3
		ADDVALUE("FP","",count); //5,5
		ADDVALUE("VOLUME","",count); //3,16
		ADDVALUE("SALES","",count); //3,12
		ADDVALUE("AMOUNT","",count); //3,12
		ADDVALUE("PRICE","",count); //3,12
		ADDVALUE("DISCOUNT","",count); //3,12
		ADDVALUE("ticket_id","",count); //4,10
		ADDVALUE("TRANSNUM","''",count); //12,8
