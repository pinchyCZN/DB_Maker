int fill_pb_mixm(CString dbname)
{
	DBMaker dbm;
	dbm.table_name="pb_mixm";
...
//	get_table_index(&db,"Config","VEHIINTKEY",&int_key);
	...
	{
		ADDVALUE("table_num","%i",count); //2,4
		ADDVALUE("type","'P'",count); //1,1
		ADDVALUE("qty","%i",count); //2,5
		ADDVALUE("price","%i",count); //2,8
		ADDVALUE("strictlow","%i",count%2); //-7,1
		ADDVALUE("stricthigh","%i",count%2); //-7,1
		ADDVALUE("c_strictlo","'T'",count); //1,1
		ADDVALUE("c_stricthi","'T'",count); //1,1
